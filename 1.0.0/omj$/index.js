...
const $mediaPreview = (elementToWatch, placeToPreview, other = {}) => {
    ...
    let operation = other.operation ?? other.then ?? (() => "operation");
        ...
        if (type === 1) {
            ...
            $on(reader, "load", (() => {
                if (elementToWatch.value !== "") {
                    ...
                    if (operation !== "operation") operation(reader.result, placeToPreview, elementToWatch);
                } else placeToPreview.src = previewPlaceholder;
            }), "on");
            reader.readAsDataURL(elementToWatch.files[0]);
        } else if (type === 2) {
            placeToPreview.src = elementToWatch.value !== "" ? elementToWatch.value : previewPlaceholder;
            if (operation !== "operation") operation(placeToPreview.src, placeToPreview, elementToWatch);
        } else {
            ...
            if (operation !== "operation") operation(srcProcessed, placeToPreview, elementToWatch);
        }
    };
    ...
    if (boxToDraw === "all" || boxToDraw === "dialog" || boxToDraw === "modal") {
        ...
        const BOX_SIZE = size => {
            switch (size) {
              ...
              case "xxl":
                BOX_INNER_WRAPPER.style.minWidth = "97vw";
                break;
